...
/backtest <symbol> [days] [timeframes] - Zaman dilimi backtest'i çalıştır
/combo_backtest <symbol> [days] - İkili kombinasyon backtest'i çalıştır
/tp_sl_backtest <symbol> [days] - TP/SL kombinasyon backtest'i çalıştır
...
async def setup_bot():
    ...
    app.add_handler(CommandHandler("combo_backtest", run_combination_backtest_command))
    app.add_handler(CommandHandler("tp_sl_backtest", run_tp_sl_backtest_command))
...
async def backtest_timeframes(symbol, start_date, end_date, test_timeframes=None):
    """
    Farklı zaman dilimlerinde backtest yapar ve en iyi performans gösteren zaman dilimini bulur
    
    Args:
        symbol: Test edilecek sembol (örn: 'BTCUSDT')
        start_date: Başlangıç tarihi (datetime)
        end_date: Bitiş tarihi (datetime)
        test_timeframes: Test edilecek zaman dilimleri listesi
    
    Returns:
        dict: Her zaman dilimi için performans sonuçları
    """
    
    if test_timeframes is None:
        test_timeframes = {
            '1m': '1m',
            '3m': '3m', 
            '5m': '5m',
    ...
    print(f"🔍 {symbol} için zaman dilimi backtest'i başlatılıyor...")
    print(f"   Başlangıç: {start_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"   Bitiş: {end_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"   Test edilecek zaman dilimleri: {list(test_timeframes.keys())}")
    
    results = {}
    
    for tf_name, tf_value in test_timeframes.items():
        try:
            print(f"\n📊 {tf_name} zaman dilimi test ediliyor...")
            
            # Tarih aralığındaki veriyi al (Binance API limit: 1000)
            df = await async_get_historical_data(symbol, tf_value, 1000)
            if df is None or df.empty:
                print(f"⚠️ {tf_name} için veri bulunamadı")
                continue
            ...
            if df.empty:
                print(f"⚠️ {tf_name} için belirtilen tarih aralığında veri bulunamadı")
                continue
            
            print(f"   {len(df)} mum verisi bulundu")
            
            # Sinyalleri hesapla
            df = calculate_full_pine_signals(df, tf_name)
            
            # Backtest sonuçlarını hesapla
            backtest_result = calculate_backtest_performance(df, symbol, tf_name)
            results[tf_name] = backtest_result
            
            print(f"✅ {tf_name} backtest tamamlandı")
            print(f"   Toplam sinyal: {backtest_result['total_signals']}")
            print(f"   Başarılı: {backtest_result['successful_signals']}")
            print(f"   Başarısız: {backtest_result['failed_signals']}")
            print(f"   Başarı oranı: %{backtest_result['success_rate']:.2f}")
            print(f"   Toplam kar/zarar: ${backtest_result['total_pnl']:.2f}")
            
        except Exception as e:
            print(f"❌ {tf_name} backtest hatası: {e}")
            results[tf_name] = None
    
    # En iyi performans gösteren zaman dilimini bul
    best_timeframe = find_best_timeframe(results)
    
    print(f"\n🏆 EN İYİ ZAMAN DİLİMİ: {best_timeframe}")
    if best_timeframe in results and results[best_timeframe]:
        best_result = results[best_timeframe]
        print(f"   Başarı oranı: %{best_result['success_rate']:.2f}")
        print(f"   Toplam kar/zarar: ${best_result['total_pnl']:.2f}")
        print(f"   Toplam sinyal: {best_result['total_signals']}")
    
    return results, best_timeframe

def calculate_backtest_performance(df, symbol, timeframe):
    """
    Backtest performansını hesaplar
    
    Args:
        df: Sinyal hesaplanmış DataFrame
        symbol: Sembol adı
    ...
    try:
        ...
        signal_changes = signal_changes[signal_changes['signal'] != 0]  # 0 olmayan sinyalleri al
        ...
        for idx, row in signal_changes.iterrows():
            ...
            if current_signal is None:
                # İlk sinyal - pozisyon aç
                current_signal = signal
                entry_price = price
                entry_time = time
            else:
                # Sinyal değişimi - pozisyon kapat
                if current_signal == 1:  # Long pozisyon
                    if signal == -1:  # Satış sinyali
                        exit_price = price
                        ...
                        entry_price = price
                        entry_time = time
                elif current_signal == -1:  # Short pozisyon
                    if signal == 1:  # Alış sinyali
                        exit_price = price
        ...
        # Son pozisyonu kapat (eğer açık kaldıysa)
        ...
        successful_signals = sum(1 for trade in trades if trade['success'])
        ...
        print(f"❌ {symbol} {timeframe} performans hesaplama hatası: {e}")
        return None

def find_best_timeframe(results):
    """
    En iyi performans gösteren zaman dilimini bulur
    
    Args:
        results: Her zaman dilimi için performans sonuçları
    
    Returns:
        str: En iyi zaman dilimi
    """
    ...
    try:
        ...
        for tf, result in valid_results.items():
            ...
            # Başarı oranı puanı (0-40 puan)
            success_score = result['success_rate'] * 0.4
            
            # Toplam kar/zarar puanı (0-30 puan)
            pnl_score = min(max(result['total_pnl'] * 2, 0), 30)  # -15% ile +15% arası
            
            # Sinyal sayısı puanı (0-20 puan)
            signal_score = min(result['total_signals'] * 0.5, 20)
            
            # Kazanma serisi puanı (0-10 puan)
            win_streak_score = min(result['win_streak'] * 2, 10)
            
            total_score = success_score + pnl_score + signal_score + win_streak_score
            scores[tf] = total_score
        
        # En yüksek puanlı zaman dilimini bul
        best_tf = max(scores, key=scores.get)
        
        print(f"\n📊 Zaman Dilimi Puanları:")
        for tf, score in sorted(scores.items(), key=lambda x: x[1], reverse=True):
            result = valid_results[tf]
            print(f"   {tf}: {score:.2f} puan (Başarı: %{result['success_rate']:.1f}, PnL: ${result['total_pnl']:.2f})")
        
        return best_tf
        
    except Exception as e:
        print(f"❌ En iyi zaman dilimi bulunurken hata: {e}")
        return "Hata"

async def run_backtest_command(update, context):
    """
    Telegram komutu ile backtest çalıştırır
    """
    try:
        user_id, is_valid = validate_user_command(update, require_admin=True)
        if not is_valid:
            await send_command_response(update, "❌ Bu komutu kullanma yetkiniz yok!")
            return
        
        # Argümanları kontrol et
        if len(context.args) < 1:
            await send_command_response(update, "❌ Kullanım: /backtest <symbol> [days] [timeframes]\n\nÖrnek:\n/backtest BTCUSDT 30\n/backtest ETHUSDT 7 1m,5m,15m")
            return
        ...
        if len(context.args) > 2:
            tf_list = context.args[2].split(',')
            test_timeframes = {tf: tf for tf in tf_list}
        else:
            test_timeframes = {
                '1m': '1m',
                '3m': '3m', 
        ...
        await send_command_response(update, f"🔍 {symbol} için {days} günlük backtest başlatılıyor...\nZaman dilimleri: {', '.join(test_timeframes.keys())}")
        ...
        # Backtest'i çalıştır
        results, best_tf = await backtest_timeframes(symbol, start_date, end_date, test_timeframes)
        
        # Sonuçları formatla
        message = f"🏆 {symbol} BACKTEST SONUÇLARI ({days} gün)\n\n"
        message += f"📅 Tarih: {start_date.strftime('%Y-%m-%d')} - {end_date.strftime('%Y-%m-%d')}\n"
        message += f"🥇 En İyi Zaman Dilimi: {best_tf}\n\n"
        ...
        for tf, result in results.items():
            ...
            message += f"📊 {tf}:\n"
            message += f"   Sinyal: {result['total_signals']}\n"
            message += f"   Başarı: %{result['success_rate']:.1f}\n"
            message += f"   PnL: ${result['total_pnl']:.2f}\n"
            message += f"   Ort. Kar: %{result['avg_profit']:.2f}\n"
            message += f"   Ort. Zarar: %{result['avg_loss']:.2f}\n\n"
        
        await send_command_response(update, message)
        
    except Exception as e:
        error_msg = f"❌ Backtest hatası: {e}"
        await send_command_response(update, error_msg)
        print(f"❌ Backtest komut hatası: {e}")

async def backtest_timeframe_combinations(symbol, start_date, end_date, test_combinations=None):
    """
    Farklı zaman dilimi ikili kombinasyonlarında backtest yapar ve en iyi performans gösteren kombinasyonu bulur
    
    Args:
        symbol: Test edilecek sembol (örn: 'BTCUSDT')
        start_date: Başlangıç tarihi (datetime)
        end_date: Bitiş tarihi (datetime)
        test_combinations: Test edilecek kombinasyonlar listesi
    
    Returns:
        dict: Her kombinasyon için performans sonuçları
    """
    ...
    if test_combinations is None:
        ...
        test_combinations = list(combinations(all_timeframes, 2))
        print(f"   Varsayılan olarak tüm {len(test_combinations)} kombinasyon oluşturuldu")
    
    print(f"🔍 {symbol} için ikili kombinasyon backtest'i başlatılıyor...")
    print(f"   Başlangıç: {start_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"   Bitiş: {end_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"   Test edilecek kombinasyonlar: {len(test_combinations)} adet")
    
    results = {}
    
    for tf1, tf2 in test_combinations:
        try:
            print(f"\n📊 {tf1} + {tf2} kombinasyonu test ediliyor...")
            
            # Her iki zaman dilimi için veri al
            df1 = await async_get_historical_data(symbol, tf1, 1000)
            df2 = await async_get_historical_data(symbol, tf2, 1000)
            
            if df1 is None or df1.empty or df2 is None or df2.empty:
                print(f"⚠️ {tf1} veya {tf2} için veri bulunamadı")
                continue
            
            # Tarih filtreleme
            df1['open_time'] = pd.to_datetime(df1['timestamp'], unit='ms')
            df2['open_time'] = pd.to_datetime(df2['timestamp'], unit='ms')
            
            df1 = df1[(df1['open_time'] >= start_date) & (df1['open_time'] <= end_date)]
            df2 = df2[(df2['open_time'] >= start_date) & (df2['open_time'] <= end_date)]
            
            if df1.empty or df2.empty:
                print(f"⚠️ {tf1} veya {tf2} için belirtilen tarih aralığında veri bulunamadı")
                continue
            
            print(f"   {tf1}: {len(df1)} mum, {tf2}: {len(df2)} mum")
            
            # Sinyalleri hesapla
            df1 = calculate_full_pine_signals(df1, tf1)
            df2 = calculate_full_pine_signals(df2, tf2)
            
            # İkili kombinasyon backtest sonuçlarını hesapla
            backtest_result = calculate_combination_backtest_performance(df1, df2, symbol, f"{tf1}+{tf2}")
            results[f"{tf1}+{tf2}"] = backtest_result
            
            print(f"✅ {tf1} + {tf2} backtest tamamlandı")
            print(f"   Toplam sinyal: {backtest_result['total_signals']}")
            print(f"   Başarılı: {backtest_result['successful_signals']}")
            print(f"   Başarısız: {backtest_result['failed_signals']}")
            print(f"   Başarı oranı: %{backtest_result['success_rate']:.2f}")
            ...
            print(f"❌ {tf1} + {tf2} backtest hatası: {e}")
            results[f"{tf1}+{tf2}"] = None
    
    # En iyi performans gösteren kombinasyonu bul
    best_combination = find_best_combination(results)
    
    print(f"\n🏆 EN İYİ İKİLİ KOMBİNASYON: {best_combination}")
    if best_combination in results and results[best_combination]:
...
def calculate_combination_backtest_performance(df1, df2, symbol, combination_name):
    """
    İkili kombinasyon backtest performansını hesaplar (2/2 kuralı)
    
    Args:
        df1: İlk zaman dilimi DataFrame
        df2: İkinci zaman dilimi DataFrame
        symbol: Sembol adı
        combination_name: Kombinasyon adı
    ...
    try:
        # Her iki timeframe'de sinyal değişimlerini bul
        signal_changes1 = df1[df1['signal'] != df1['signal'].shift(1)]
        signal_changes1 = signal_changes1[signal_changes1['signal'] != 0]
        
        signal_changes2 = df2[df2['signal'] != df2['signal'].shift(1)]
        signal_changes2 = signal_changes2[signal_changes2['signal'] != 0]
        
        if signal_changes1.empty or signal_changes2.empty:
            return {
                'total_signals': 0,
                'successful_signals': 0,
                'failed_signals': 0,
                ...
                'win_streak': 0,
                'lose_streak': 0,
                'combination_agreement': 0.0
            }
        
        # 2/2 kuralı için sinyal uyumu kontrolü
        trades = []
        current_signal1 = None
        current_signal2 = None
        entry_price = None
        entry_time = None
        
        # Her iki timeframe'deki sinyal değişimlerini birleştir ve sırala
        all_changes = []
        
        for idx, row in signal_changes1.iterrows():
            all_changes.append({
                'time': row['open_time'],
                'signal': row['signal'],
                'price': float(row['close']),
        ...
        for idx, row in signal_changes2.iterrows():
            all_changes.append({
                'time': row['open_time'],
                'signal': row['signal'],
                'price': float(row['close']),
        ...
        for change in all_changes:
            if change['timeframe'] == 'tf1':
                current_signal1 = change['signal']
            else:
                current_signal2 = change['signal']
            
            # Her iki timeframe'de de sinyal varsa kontrol et
            if current_signal1 is not None and current_signal2 is not None:
                # 2/2 kuralı: Her iki timeframe de aynı yönde olmalı
                if current_signal1 == current_signal2:
                    # Pozisyon aç
                    if entry_price is None:
                        entry_price = change['price']
                        entry_time = change['time']
                        signal_type = current_signal1
                    else:
                        # Pozisyon kapat (sinyal değişimi)
                        exit_price = change['price']
                        
                        if signal_type == 1:  # Long pozisyon
                            pnl = (exit_price - entry_price) / entry_price * 100
                        else:  # Short pozisyon
                            pnl = (entry_price - exit_price) / entry_price * 100
                        ...
                        trades.append({
                            ...
                            'pnl': pnl,
                            'entry_time': entry_time,
                            'exit_time': change['time'],
                            'success': pnl > 0,
                            'tf1_signal': current_signal1,
                            'tf2_signal': current_signal2
                        })
                        
                        # Yeni pozisyon için hazırla
                        entry_price = change['price']
                        entry_time = change['time']
                        signal_type = current_signal1
        ...
        if entry_price is not None and entry_time is not None:
            last_price = float(df1.iloc[-1]['close'])  # df1'den son fiyatı al
            if signal_type == 1:  # Long pozisyon
                pnl = (last_price - entry_price) / entry_price * 100
            else:  # Short pozisyon
                pnl = (entry_price - last_price) / entry_price * 100
            ...
            trades.append({
                ...
                'pnl': pnl,
                'entry_time': entry_time,
                'exit_time': df1.iloc[-1]['open_time'],
                'success': pnl > 0,
                'tf1_signal': current_signal1,
                'tf2_signal': current_signal2
            })
        ...
        # Kombinasyon uyumu hesapla
        agreement_count = sum(1 for trade in trades if trade['tf1_signal'] == trade['tf2_signal'])
        combination_agreement = (agreement_count / total_signals) * 100 if total_signals > 0 else 0
        ...
        return {
            ...
            'win_streak': win_streak,
            'lose_streak': lose_streak,
            'combination_agreement': combination_agreement,
            'trades': trades
        }
        
    except Exception as e:
        print(f"❌ {symbol} {combination_name} kombinasyon performans hesaplama hatası: {e}")
        return None

def find_best_combination(results):
    """
    En iyi performans gösteren ikili kombinasyonu bulur
    
    Args:
        results: Her kombinasyon için performans sonuçları
    
    Returns:
        str: En iyi kombinasyon
    """
    ...
    try:
        ...
        for combo, result in valid_results.items():
            if result['total_signals'] == 0:
                scores[combo] = 0
                continue
            
            # Başarı oranı puanı (0-35 puan)
            success_score = result['success_rate'] * 0.35
            
            # Toplam kar/zarar puanı (0-25 puan)
            pnl_score = min(max(result['total_pnl'] * 2, 0), 25)
            
            # Sinyal sayısı puanı (0-20 puan)
            signal_score = min(result['total_signals'] * 0.4, 20)
            
            # Kombinasyon uyumu puanı (0-15 puan)
            agreement_score = result['combination_agreement'] * 0.15
            
            # Kazanma serisi puanı (0-5 puan)
            win_streak_score = min(result['win_streak'] * 1, 5)
            
            total_score = success_score + pnl_score + signal_score + agreement_score + win_streak_score
            scores[combo] = total_score
        
        # En yüksek puanlı kombinasyonu bul
        best_combo = max(scores, key=scores.get)
        
        print(f"\n📊 Kombinasyon Puanları:")
        for combo, score in sorted(scores.items(), key=lambda x: x[1], reverse=True):
            result = valid_results[combo]
            print(f"   {combo}: {score:.2f} puan (Başarı: %{result['success_rate']:.1f}, PnL: ${result['total_pnl']:.2f}, Uyum: %{result['combination_agreement']:.1f})")
        
        return best_combo
        
    except Exception as e:
        print(f"❌ En iyi kombinasyon bulunurken hata: {e}")
        return "Hata"

async def run_combination_backtest_command(update, context):
    """
    Telegram komutu ile ikili kombinasyon backtest çalıştırır
    """
    try:
        user_id, is_valid = validate_user_command(update, require_admin=True)
        if not is_valid:
        ...
        # Argümanları kontrol et
        if len(context.args) < 1:
            await send_command_response(update, "❌ Kullanım: /combo_backtest <symbol> [days]\n\nÖrnek:\n/combo_backtest BTCUSDT 30")
            return
        
        symbol = context.args[0].upper()
        days = int(context.args[1]) if len(context.args) > 1 else 30
        
        await send_command_response(update, f"🔍 {symbol} için {days} günlük ikili kombinasyon backtest başlatılıyor...")
        
        # Tarih aralığını hesapla
        end_date = datetime.now()
        start_date = end_date - timedelta(days=days)
        
        # Kombinasyon backtest'i çalıştır
        results, best_combo = await backtest_timeframe_combinations(symbol, start_date, end_date)
        
        # Sonuçları formatla
        message = f"🏆 {symbol} İKİLİ KOMBİNASYON BACKTEST SONUÇLARI ({days} gün)\n\n"
        message += f"📅 Tarih: {start_date.strftime('%Y-%m-%d')} - {end_date.strftime('%Y-%m-%d')}\n"
        message += f"🥇 En İyi Kombinasyon: {best_combo}\n\n"
        
        for combo, result in results.items():
            if result is None:
                message += f"❌ {combo}: Hata\n"
                continue
            ...
            message += f"   Başarı: %{result['success_rate']:.1f}\n"
            message += f"   PnL: ${result['total_pnl']:.2f}\n"
            message += f"   Uyum: %{result['combination_agreement']:.1f}\n"
            message += f"   Ort. Kar: %{result['avg_profit']:.2f}\n"
            message += f"   Ort. Zarar: %{result['avg_loss']:.2f}\n\n"
        ...
        error_msg = f"❌ Kombinasyon backtest hatası: {e}"
        await send_command_response(update, error_msg)
        print(f"❌ Kombinasyon backtest komut hatası: {e}")

def calculate_backtest_performance_with_tp_sl(df, symbol, timeframe, tp_percent=3.0, sl_percent=1.0):
    """
    TP/SL ile backtest performansını hesaplar (TEK TP ve TEK SL)
    
    Args:
        df: Sinyal hesaplanmış DataFrame
        symbol: Sembol adı
        timeframe: Zaman dilimi
        tp_percent: Take Profit yüzdesi (varsayılan %3)
        sl_percent: Stop Loss yüzdesi (varsayılan %1)
    ...
    try:
        ...
        for idx, row in signal_changes.iterrows():
            ...
            # TP ve SL hesapla
            if signal == 1:  # ALIŞ sinyali
                tp_price = entry_price * (1 + tp_percent / 100)
                sl_price = entry_price * (1 - sl_percent / 100)
            elif signal == -1:  # SATIŞ sinyali
                tp_price = entry_price * (1 - tp_percent / 100)
                sl_price = entry_price * (1 + sl_percent / 100)
            else:
                continue
            
            # Bu sinyalden sonraki mumları kontrol et
            remaining_df = df[df.index > entry_idx]
            
            exit_price = None
            exit_time = None
            exit_type = None
            
            for exit_idx, exit_row in remaining_df.iterrows():
                high = float(exit_row['high'])
                low = float(exit_row['low'])
                close = float(exit_row['close'])
                
                if signal == 1:  # ALIŞ pozisyonu
                    # Önce TP kontrolü (kar alma öncelikli)
                    if high >= tp_price:
                        exit_price = tp_price
            ...
            # Eğer TP/SL tetiklenmemişse son fiyattan çık
            if exit_price is None:
                last_row = remaining_df.iloc[-1] if not remaining_df.empty else row
        ...
        total_signals = len(trades)
        successful_signals = len(successful_trades)
        failed_signals = len(failed_trades)
        success_rate = (successful_signals / total_signals) * 100 if total_signals > 0 else 0
        
        total_pnl = sum(t['pnl'] for t in trades)
        avg_profit = sum(t['pnl'] for t in successful_trades) / len(successful_trades) if successful_trades else 0
        avg_loss = sum(t['pnl'] for t in failed_trades) / len(failed_trades) if failed_trades else 0
        max_profit = max((t['pnl'] for t in trades), default=0)
        max_loss = min((t['pnl'] for t in trades), default=0)
        
        # TP/SL istatistikleri
        ...
        print(f"❌ TP/SL Backtest performans hesaplama hatası: {e}")
        return {
            'total_signals': 0,
            'successful_signals': 0,
            'failed_signals': 0,
...
async def backtest_single_tp_sl(symbol, start_date, end_date, test_combinations=None):
    """
    Farklı TP/SL kombinasyonlarında backtest yapar ve en iyi performans gösteren TP/SL'yi bulur
    
    Args:
        symbol: Test edilecek sembol (örn: 'BTCUSDT')
        start_date: Başlangıç tarihi (datetime)
        end_date: Bitiş tarihi (datetime)
        test_combinations: Test edilecek TP/SL kombinasyonları [(tp, sl), ...]
    
    Returns:
        dict: Her TP/SL kombinasyonu için performans sonuçları
    """
    
    if test_combinations is None:
        # Varsayılan TP/SL kombinasyonları
        test_combinations = [
            (1.0, 0.5),   # TP %1, SL %0.5
            (1.5, 0.5),   # TP %1.5, SL %0.5
            (2.0, 0.5),   # TP %2, SL %0.5
            (2.0, 1.0),   # TP %2, SL %1
            (3.0, 1.0),   # TP %3, SL %1
            (3.0, 1.5),   # TP %3, SL %1.5
    ...
    print(f"🔍 {symbol} için TP/SL kombinasyon backtest'i başlatılıyor...")
    print(f"   Başlangıç: {start_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"   Bitiş: {end_date.strftime('%Y-%m-%d %H:%M')}")
    print(f"   Test edilecek TP/SL kombinasyonları: {len(test_combinations)} adet")
    
    # 15m+30m kombinasyonu için veriyi al
    timeframe = '15m'
    df = await async_get_historical_data(symbol, timeframe, 1000)
    if df is None or df.empty:
        print(f"⚠️ {symbol} için {timeframe} veri bulunamadı")
        return {}
    ...
    if df.empty:
        print(f"⚠️ {symbol} için belirtilen tarih aralığında veri bulunamadı")
        return {}
    
    print(f"   {len(df)} mum verisi bulundu")
    
    # Sinyalleri hesapla
    df = calculate_full_pine_signals(df, timeframe)
    
    results = {}
    
    for tp_percent, sl_percent in test_combinations:
        try:
            print(f"\n📊 TP %{tp_percent} / SL %{sl_percent} test ediliyor...")
            
            # TP/SL ile backtest yap
            backtest_result = calculate_backtest_performance_with_tp_sl(
                df, symbol, f"{timeframe}_TP{tp_percent}_SL{sl_percent}", tp_percent, sl_percent
            )
            
            combination_key = f"TP{tp_percent}_SL{sl_percent}"
            results[combination_key] = backtest_result
            
            print(f"✅ TP %{tp_percent} / SL %{sl_percent} backtest tamamlandı")
            print(f"   Toplam sinyal: {backtest_result['total_signals']}")
            print(f"   Başarılı: {backtest_result['successful_signals']}")
            print(f"   Başarısız: {backtest_result['failed_signals']}")
            print(f"   Başarı oranı: %{backtest_result['success_rate']:.2f}")
            print(f"   Toplam kar/zarar: %{backtest_result['total_pnl']:.2f}")
            print(f"   TP tetikleme: {backtest_result['tp_count']}")
            print(f"   SL tetikleme: {backtest_result['sl_count']}")
            print(f"   Timeout: {backtest_result['timeout_count']}")
            
        except Exception as e:
            print(f"❌ TP %{tp_percent} / SL %{sl_percent} backtest hatası: {e}")
            results[f"TP{tp_percent}_SL{sl_percent}"] = None
    
    # En iyi performans gösteren TP/SL kombinasyonunu bul
    best_combination = find_best_tp_sl_combination(results)
    
    print(f"\n🏆 EN İYİ TP/SL KOMBİNASYONU: {best_combination}")
    if best_combination in results and results[best_combination]:
        best_result = results[best_combination]
        print(f"   Başarı oranı: %{best_result['success_rate']:.2f}")
        print(f"   Toplam kar/zarar: %{best_result['total_pnl']:.2f}")
        print(f"   Toplam sinyal: {best_result['total_signals']}")
        print(f"   TP: %{best_result['tp_percent']}, SL: %{best_result['sl_percent']}")
    
    return results

def find_best_tp_sl_combination(results):
    """En iyi TP/SL kombinasyonunu bulur"""
    if not results:
        return None
    ...
    for combination, result in results.items():
        ...
        # Skor hesaplama: Başarı oranı (40%) + Toplam PnL (40%) + Sinyal sayısı (20%)
        success_rate_score = result['success_rate'] * 0.4
        pnl_score = result['total_pnl'] * 0.4
        signal_count_score = min(result['total_signals'] / 100 * 100, 100) * 0.2  # Max 100 puan
        
        total_score = success_rate_score + pnl_score + signal_count_score
...
async def run_tp_sl_backtest_command(update, context):
    """
    Telegram komutu ile TP/SL backtest çalıştırır
    """
    try:
        user_id, is_valid = validate_user_command(update, require_admin=True)
        if not is_valid:
        ...
        # Argümanları kontrol et
        if len(context.args) < 1:
            await send_command_response(update, "❌ Kullanım: /tp_sl_backtest <symbol> [days]\n\nÖrnek:\n/tp_sl_backtest BTCUSDT 30")
            return
        ...
        await send_command_response(update, f"🎯 {symbol} TP/SL backtest başlatılıyor...\n📅 Son {days} gün")
        
        # TP/SL Backtest'i çalıştır
        results = await backtest_single_tp_sl(symbol, start_date, end_date)
        
        if not results:
            await send_command_response(update, f"❌ {symbol} için TP/SL backtest sonucu bulunamadı")
            return
        
        # En iyi 10 TP/SL kombinasyonunu göster
        sorted_results = []
        for combo, result in results.items():
            if result and result['total_signals'] > 0:
                ...
                success_rate_score = result['success_rate'] * 0.4
                pnl_score = result['total_pnl'] * 0.4
                signal_count_score = min(result['total_signals'] / 100 * 100, 100) * 0.2
                total_score = success_rate_score + pnl_score + signal_count_score
        ...
        # Sonuçları formatla
        response = f"🎯 {symbol} TP/SL BACKTEST SONUÇLARI\n"
        response += f"📅 Son {days} gün\n\n"
        response += "🏆 EN İYİ 10 TP/SL KOMBİNASYONU:\n\n"
        ...
        for i, item in enumerate(sorted_results[:10], 1):
            ...
            response += f"{i}. {combo}\n"
            response += f"   📈 Başarı: %{result['success_rate']:.1f}\n"
            response += f"   💰 PnL: %{result['total_pnl']:.2f}\n"
            response += f"   📊 Sinyal: {result['total_signals']}\n"
            response += f"   🎯 TP: {result['tp_count']} | 🛑 SL: {result['sl_count']}\n"
            response += f"   ⏰ Timeout: {result['timeout_count']}\n"
            response += f"   ⭐ Skor: {score:.1f}\n\n"
        
        await send_command_response(update, response)
        
    except Exception as e:
        error_msg = f"❌ TP/SL backtest hatası: {e}"
        await send_command_response(update, error_msg)
        print(f"❌ TP/SL backtest komut hatası: {e}")
